...
class DefaultTrainer(SimpleTrainer):
    ...
    def resume_or_load(self, resume=True):
        ...
        #copy module's weight
        if self.cfg.MODEL.ROI_HEADS.NOVEL_MODULE.COPY_OPTION == True:

            #feature extractor
            for i in range(len(self.model.module.roi_heads.box_head.fcs)):
                dic = self.model.module.roi_heads.box_head.fcs[i].state_dict()
                self.model.module.roi_heads.novel_module.feature_extractor.fcs[i].load_state_dict(dic)
