...
from .meta_rfs import register_meta_rfs
...
def register_all_pascal_voc(root="/media/datasets/PascalVOC_outline/VOCdevkit"):
    SPLITS = [
...
def register_all_rfs(root="/media/datasets/RFS"):
    ...
    for name, dirname, split in SPLITS:
        year = 2021
        register_rfs(name, os.path.join(root, dirname), split, year)
        MetadataCatalog.get(name).evaluator_type = "rfs"

    # register meta datasets
    METASPLITS = [
        ("rfs_trainval_base1", "train", "trainval", "base1", 1),
        ("rfs_trainval_base2", "train", "trainval", "base2", 2),
        ("rfs_trainval_base3", "train", "trainval", "base3", 3),
        ("rfs_trainval_all1", "train", "trainval", "base_novel_1", 1),
        ("rfs_trainval_all2", "train", "trainval", "base_novel_2", 2),
        ("rfs_trainval_all3", "train", "trainval", "base_novel_3", 3),
        ("rfs_test_base1", "test", "test", "base1", 1),
        ("rfs_test_base2", "test", "test", "base2", 2),
        ("rfs_test_base3", "test", "test", "base3", 3),
        ("rfs_test_novel1", "test", "test", "novel1", 1),
        ("rfs_test_novel2", "test", "test", "novel2", 2),
        ("rfs_test_novel3", "test", "test", "novel3", 3),
        ("rfs_test_all1", "test", "test", "base_novel_1", 1),
        ("rfs_test_all2", "test", "test", "base_novel_2", 2),
        ("rfs_test_all3", "test", "test", "base_novel_3", 3),
    ]
        ...
        for sid in range(1, 4):
            for shot in [1, 2, 3, 5, 10]:
                for seed in range(20):
                    # seed = '' if seed == 0 else '_seed{}'.format(seed)
                    seed = '_seed{}'.format(seed)
                    name = "rfs_trainval_{}{}_{}shot{}".format(
    ...
    # ('voc_2007_trainval_novel3_10shot', 'VOC2007', 'novel_10shot_split_3_trainval', 'novel3', 3)
    for name, dirname, split, keepclasses, sid in METASPLITS:
        register_meta_rfs(name,
                                 _get_builtin_metadata("rfs_fewshot"),
                                 os.path.join(root, dirname), split, 2021,
                                 keepclasses, sid)
        MetadataCatalog.get(name).evaluator_type = "rfs"
    ...
    for prefix in ["all", "novel"]:
        for sid in range(1, 4):
            for shot in [1, 2, 3, 5, 10]:
                for seed in range(20):
                    seed = '_seed{}'.format(seed)
                    name = "rfs_trainval_{}{}_{}shot{}_{}ploidy".format(
                        prefix, sid, shot, seed, ploidy)
                    dirname = "train"
                    img_file = "{}_{}shot_split_{}_trainval".format(
                        prefix, shot, sid)
                    keepclasses = "base_novel_{}".format(sid) \
