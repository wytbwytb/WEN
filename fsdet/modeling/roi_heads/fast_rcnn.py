...
from ..novel_module import prototypes_loss
...
class FastRCNNNovelOutputs(FastRCNNOutputs):
    ...
    def __init__(
        ...
        super(FastRCNNNovelOutputs,self).__init__(box2box_transform,pred_class_logits,pred_proposal_deltas,proposals,smooth_l1_beta)
        self.all_class_prototype_features = all_class_prototype_features
        self.proloss_factor = proloss_factor


    def softmax_cross_entropy_loss_for_pre(self):
        """
        Compute the softmax cross entropy loss for box classification.
        ...
        """
        self._log_accuracy()
        return F.cross_entropy(self.pre_pred_class_logits, self.gt_classes, reduction="mean")

    def losses(self):
        if self.all_class_prototype_features == None:
            return {
            'loss_cls': self.softmax_cross_entropy_loss(),
            'loss_box_reg': self.smooth_l1_loss()
        }
        ...
        return {
            'loss_pro': prototypes_loss(self.all_class_prototype_features, self.proloss_factor),
            'loss_cls': self.softmax_cross_entropy_loss(),
            'loss_box_reg': self.smooth_l1_loss()
        }
...
@ROI_HEADS_OUTPUT_REGISTRY.register()
class CosineSimCLSLayer(nn.Module):
    """
    One outputs
    (1) classification score is based on cosine_similarity
    """

    def __init__(
        self, cfg, input_size, num_classes, box_dim=4
    ):
        """
        Args:
            cfg: config
            ...
            box_dim (int): the dimension of bounding boxes.
                Example box dimensions: 4 for regular XYXY boxes and 5 for rotated XYWHA boxes
        """
        super(CosineSimCLSLayer, self).__init__()
